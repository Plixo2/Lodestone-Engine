...
public class TabViewport extends UITab {
    ...
    public void save() {
        if(this.script != null) {
            FunctionManager.saveToFile(script);
        }
    }
    ...
    public void mouseClicked(float mouseX, float mouseY, int mouseButton) {
        ...
        if(mouseButton == 1) {
            OptionMenu.TxtRun[] functions = new OptionMenu.TxtRun[FunctionManager.functions.size()];
            for (int i = 0; i < FunctionManager.functions.size(); i++) {
                nFunction function = FunctionManager.functions.get(i);
                functions[i] = new OptionMenu.TxtRun(function.getName()) {
                    @Override
                    public void run() {
                        if(script != null) {
                            nFunction nFunction = FunctionManager.getInstanceByName(function.getName());
                            if(nFunction != null) {
                                script.addFunction(nFunction,mouseToWorld.x,mouseToWorld.y);
                                canvas.add(nFunction.ui);
                            }
                        }
                    }
                };
            }
            showMenu(0,mouseX,mouseY, functions);
        }
    ...
    public void keyPressed(int key, int scanCode, int action) {
        if(key == GLFW.GLFW_KEY_DELETE) {
            for (UIElement element : canvas.elements) {
                ...
                nUIFunction nUIFunction = ((nUIFunction) element);
                if(nUIFunction.isSelected) {
                    canvas.elements.remove(element);
                    script.removeFunction(nUIFunction.function);
                }
            }
        }
