...
import net.plixo.paper.client.avs.components.variable.GetGlobalVariable;
...
/*
    ...
    public void init() {
        ...
        head = new Rect(0, 0, getWidth(), headerHeight, mainColor, ColorLib.interpolateColor(mainColor, 0xFFFFFFFF, 0.1f));
    ...
    boolean mouseClicked(Canvas tab, int i, float mouseX, float mouseY, int mouseButton) {
        ...
        /*
        if (mouseButton != 0) {
           // VisualScriptManager.draggedType = 0;
            return false;
        }
        ...
        try {
            if (function instanceof Execute) {
                ...
                for (Rect rect : rights) {
                    if (rect.mouseInside(mouseX - x, mouseY - y, mouseButton)) {
                        if (!KeyboardUtil.isKeyDown(GLFW.GLFW_KEY_DELETE)) {
                            if (VisualScriptManager.draggedType == 0) {
                                VisualScriptManager.dragTab = i;
                                VisualScriptManager.dragIndex = rect.custom.getX();
                ...
                if (leftConnection != null && VisualScriptManager.draggedType == -1) {
                    if (leftConnection.mouseInside(mouseX - x, mouseY - y, mouseButton)) {
                        ...
                        if (mod != null) {
                            ...
                            if (tab1 != null) {
                                ...
                                int i2 = VisualScriptManager.dragIndex;
            ...
            for (Rect rect : bodies) {
                if (rect.mouseInside(mouseX - x, mouseY - y, mouseButton)) {
                    ...
                    if (VisualScriptManager.draggedType == 1 && side == -1) {
                        Module mod = TheEditor.activeMod;
                        if (mod != null) {
                            Canvas tab1 = mod.getTab();
                            if (tab1 != null) {
                    ...
                    } else if (VisualScriptManager.draggedType == 0 && side == 1) {
                        VisualScriptManager.dragTab = i;
                        VisualScriptManager.dragIndex = index;
                        VisualScriptManager.draggedType = 1;
                        return true;
                    }
                }
