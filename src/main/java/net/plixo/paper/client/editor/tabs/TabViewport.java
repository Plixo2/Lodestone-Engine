...
import net.plixo.paper.client.avs.newVersion.CursorObject;
...
public class TabViewport extends UITab {
    ...
    public void init() {
        ...
        if(AssetLoader.getLoadedScript() != null) {
            for (nFunction function : AssetLoader.getLoadedScript().getFunctions()) {
            ...
            for (int i = 0; i < FunctionManager.functions.size(); i++) {
                ...
                GUIEditor.instance.addMenuOption(function.getName() , () -> {
                    if(AssetLoader.getLoadedScript() != null) {
                        ...
                        if(nFunction != null) {
                            AssetLoader.getLoadedScript().addFunction(nFunction,mouseToWorld.x,mouseToWorld.y);
        ...
        if(key == GLFW.GLFW_KEY_DELETE) {
            for (UIElement element : canvas.elements) {
                ...
                if(nUIFunction.isSelected) {
                    ...
                    AssetLoader.getLoadedScript().removeFunction(nUIFunction.function);
                }
            }
        }
